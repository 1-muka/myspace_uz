(function (global) {
    ...
    async function connectTrackerToFirestore() {
        ...
        var authUser = await global.EmeraldFirebase.ensureAuthSession();
        if (!authUser) {
            console.warn("EmeraldFirebaseBridge: Firebase auth unavailable. Using local storage.");
            return false;
        }
    ...
    async function syncLocalResultsForCurrentUser() {
        ...
        var connected = await connectTrackerToFirestore();
        if (!connected) {
            return { synced: false, reason: "auth_unavailable" };
        }
