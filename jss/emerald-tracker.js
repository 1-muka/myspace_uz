(function (global) {
    ...
    function isPracticeSectionResult(result) {
        return result && result.mode !== "full_mock";
    }
    ...
    async function getRecentResults(userId, limit) {
        ...
        var sectionResults = (await getMergedSectionResults(effectiveUserId)).filter(isPracticeSectionResult);
    ...
    async function getSectionStats(userId) {
        var effectiveUserId = userId || getCurrentUser().id;
        var results = (await getMergedSectionResults(effectiveUserId)).filter(isPracticeSectionResult);
        var groups = {};
    ...
    async function getProgressSeries(userId) {
        ...
        var sectionResults = sortByCreatedAtAsc(
            (await getMergedSectionResults(effectiveUserId)).filter(isPracticeSectionResult)
        );
    ...
    async function getLeaderboard(limit) {
        ...
        var ranked = Object.keys(userMap).map(function (userId) {
            ...
            var sectionBands = (groupedSections[userId] || []).filter(isPracticeSectionResult).map(function (item) {
    ...
    async function getPlatformInsights() {
        var users = await getMergedUsers();
        var allSections = (await getMergedAllSectionResults()).filter(isPracticeSectionResult);
        var allMocks = await getMergedAllFullMockResults();
        var latestByUser = {};
