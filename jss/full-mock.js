...
async function finishWholeMock(fromTimer) {
    ...
    const persistenceTasks = [
        (async () => {
            const writingDocs = window.EmeraldWritingDocsBridge
                ? await EmeraldWritingDocsBridge.saveFullMockWritingSubmissions({
                    username: state.candidateName || getSignedInName() || "",
                    testId: "full-mock-test1",
                    taskPrefix: "Full Mock Test 1",
                    submittedAt,
                    answers: {
    ...
    if (window.EmeraldSheetsBridge) {
        EmeraldSheetsBridge.saveFullMockResult({
            ...
            writingBand,
            overallBand,
            listeningScore: listeningAnswered,
