...
const ANSWERS_KEY = "reading_test5_answers";
const STATE_KEY = "reading_test5_state";
...
function getBand(score) {
    if (score >= 38) return 9.0;
    if (score >= 36) return 8.5;
    if (score >= 34) return 8.0;
    if (score >= 31) return 7.5;
    if (score >= 29) return 7.0;
    if (score >= 25) return 6.5;
    if (score >= 22) return 6.0;
    if (score >= 17) return 5.5;
    ...
    overlay.innerHTML = `
        ...
        ">
            ...
            <div style="font-size:20px;color:#111827;margin-bottom:8px;">
                Score: <strong>${score}/${QUESTION_COUNT}</strong>
            </div>
...
function applyReviewAttempt(attempt) {
    ...
    if (navBar) {
        ...
        banner.textContent = `Review mode: Band ${attempt.band ?? "?"} (${attempt.correctAnswers ?? 0}/${QUESTION_COUNT})`;
