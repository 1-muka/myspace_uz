const FULL_MOCK_TWO_CONFIG = {
    ...
    testId: "full-mock-test4",
    maxViolations: 3,
    audioSrc: "../audio/full-mock-test4-listening.mp3",
...
const FULL_MOCK_TWO_STATE_KEY = "fullMockTest4State";
...
const LISTENING_RADIO_QUESTIONS = [11, 12, 13, 14, 15, 16, 17, 18, 21, 22, 23, 24, 31, 32];
...
const WRITING_TASKS = [
    {
        ...
        promptHtml: `
            <div class="instruction-box">
                <p>
                    ...
                    The table shows the amount of money given to developing countries by the USA, EU countries and other countries from 2006 to 2010.
                </p>
        ...
        <img
            src="../images/full-mock-test4-task1.png"
            alt="Task 1 visual"
            class="task-visual"
        ...
        <div class="task-visual-placeholder">
                ...
                <code>../images/full-mock-test4-task1.png</code>.
                </div>
            </div>
        `
    },
        ...
        promptHtml: `
            <div class="instruction-box">
                <p class="essay-statement">
                    Some people think news has no connection to people's lives, so it is a waste of time to read the newspaper and watch television news programs.
                </p>
...
const LISTENING_ANSWER_KEY = {
    ...
    lq7: "water",
    lq8: "transportation",
    lq9: "deposit|a deposit",
    lq10: "references|uk references",
    lq11: "B",
    lq12: "A",
    lq13: "B",
    lq14: "C",
    ...
    lq37: "tutors",
    lq38: "maturity",
    lq39: "advisors|advisers",
    lq40: "online|online help"
};
...
async function finishWriting(fromTimer = false) {
    ...
    if (window.EmeraldWritingAI) {
        try {
            ...
            console.error("Failed to evaluate Full Mock Test 4 writing:", error);
        }
    }
    ...
    const writingDocsTask = window.EmeraldWritingDocsBridge
        ? EmeraldWritingDocsBridge.saveFullMockWritingSubmissions({
            username: state.candidateName || App.getUser()?.name || "",
            testId: FULL_MOCK_TWO_CONFIG.testId,
            taskPrefix: "Full Mock Test 4",
            submittedAt,
            answers: {
        ...
        : Promise.resolve({ task1Url: null, task2Url: null });

    const finalMessage = [
        fromTimer ? "The writing timer has ended." : "The full mock has been submitted.",
        "Thank you for taking the full mock test. Your result has been saved and will be released very soon."
    ...
    if (window.EmeraldSheetsBridge) {
        EmeraldSheetsBridge.saveFullMockResult({
            userName: state.candidateName || App.getUser()?.name || "",
            testId: FULL_MOCK_TWO_CONFIG.testId,
            listeningBand,
            readingBand,
    ...
    if (!window.EmeraldTracker) {
        writingDocsTask.catch((error) => {
            console.error("Failed to upload Full Mock Test 4 writing docs:", error);
        });
        return;
    }
    ...
    Promise.allSettled([
        (async () => {
            const writingDocs = await writingDocsTask;

            return EmeraldTracker.recordFullMockResult({
                testId: "test4",
                listeningBand,
                readingBand,
                writingBand,
        ...
        console.error("Failed to save Full Mock Test 4 results:", error);
    });
}
...
function applyReviewMode(attempt) {
    ...
    state.answers.listening = { ...answers };
    saveState();
    ...
    const selectedMulti = Array.isArray(answers.lq18_20) ? answers.lq18_20.map(normalizeChoice) : [];
    document.querySelectorAll('input[name="lq18_20"]').forEach((input) => {
        input.checked = selectedMulti.includes(normalizeChoice(input.value));
        input.disabled = true;
    });

    updateListeningGrid();
    elements.finishListeningBtn.textContent = "Finish Review";
    elements.finishListeningFooterBtn.textContent = "Finish Review";
    elements.finishListeningBtn.onclick = () => { window.location.href = FULL_MOCK_TWO_CONFIG.paths.dashboardPage; };
    elements.finishListeningFooterBtn.onclick = () => { window.location.href = FULL_MOCK_TWO_CONFIG.paths.dashboardPage; };
}
...
function consumeDashboardLaunch() {
    ...
    if (!launch || launch.testId !== "test4" || !launch.verified) {
        return false;
    }
